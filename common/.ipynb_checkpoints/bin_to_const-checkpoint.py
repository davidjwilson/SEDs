...
def spectrum_to_const_res(spectrum, res=1):
    ...
    if 'EXPTIME' in spectrum.dtype.names:                  
        ...
        if spectrum['WAVELENGTH'][-1] > 10000:
            ...
            new_error = np.array([], dtype=float)
            while start <= w1[-1]:
                ...
                input_spec = Spectrum1D(spectral_axis=(w2.value)*u.AA, flux=f2*u.Unit('erg cm-2 s-1 AA-1'), uncertainty= StdDevUncertainty(e2))
                ...
                new_error = np.concatenate((new_error, (1/(new_spec_fluxcon.uncertainty.array**0.5))))
                start += step
            new_flux = np.nan_to_num(new_flux, 0)
