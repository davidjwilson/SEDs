...
def make_model_metadata(new_data, normfac, sed_metadata):
    """
    ...
    meta_names =  ['TELESCOP', 'INSTRUME','GRATING','TARGNAME','RA_TARG','DEC_TARG','PROPOSID','HLSPNAME','HLSPACRN','HLSPLEAD','PR_INV_L',
                  ...
                  'FLUXMAX','FLUXUNIT']
    meta_fill = ['Model','LYA-RECONSTRUCTION','NA','','','','','','','','','',normfac,wavelength[0], wavelength[-1],'ang','vac',specres,waveres,np.min(flux[np.isnan(flux)==False]), np.max(flux[np.isnan(flux)==False]),'erg/s/cm2/ang']
    metadata = {}
...
def make_lya_spectrum(lya_path, version, sed_data ,savepath = '', save_ecsv=False, save_fits=False, normfac=1.0):
    """
    Main function.
    """
    data = get_lya_data(lya_path)
    metadata = make_model_metadata(data,  normfac, sed_data.meta)
    if save_ecsv:
