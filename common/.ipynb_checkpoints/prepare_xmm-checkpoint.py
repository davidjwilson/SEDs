...
def build_xmm_data(data, hdr0):
    ...
    start = np.full(len(w), Time(hdr0.header['pn_DATE-OBS']).mjd)
    end = np.full(len(w), Time(hdr0.header['pn_DATE-END']).mjd)
...
def build_xmm_metadata(hdr0, new_data, sed_meta):
    ...
    start, end, exptime = np.min(new_data['EXPSTART'][new_data['EXPSTART']>0]), np.max(new_data['EXPEND']), np.max(new_data['EXPTIME'])
...
def make_xmm_spectra(xmm_path, component_repo, sed_meta, apec_repo='', make_apec=True ):
    ...
    metadata = build_xmm_metadata(hdr0, new_data, sed_meta)
    if make_apec:
